...
str compileToJava(start[Oracle] oracle) =
  ...
  'import org.testar.oracle.DslOracle;
  '
...
str widget2javaForSupplier((Widget)`it`) = "";

str widget2javaForSupplier(w:(Widget)`<Name elementType> <String selector>`) = 
  "Widget <widgetVar(w)> = getWidget(\"<w.elementType>\", <w.selector>, constraintWidget);
  'if (<widgetVar(w)> == null) {
  '  return null;
  '}";

str widget2javaForSupplier(w:(Widget)`<Widget w0>.<Name fld>`) = 
  "<widget2javaForSupplier(w0)>
  'Object <widgetVar(w)> = getProperty(<widgetVar(w0)>, \"<fld>\");";
