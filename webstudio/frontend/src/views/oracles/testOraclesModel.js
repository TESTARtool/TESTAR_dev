...
export function clearedOracleSourceState() {
    return {
...
export function loadedOracleSourceState(oracleSourceFile, options = {}) {
    return {
        selectedOracleSourceFile: oracleSourceFile,
        oracleSourceDraftContent: oracleSourceFile?.content || "",
        savedOracleSourceContent: oracleSourceFile?.content || "",
        oracleDslResult: options.clearDslResult === false ? options.oracleDslResult || null : null,
        javaCompileResult: options.clearJavaCompileResult === false ? options.javaCompileResult || null : null
    };
}

export function shouldClearOracleSourceAfterDelete(selectedOracleSourceFile, deletedPath) {
    return selectedOracleSourceFile?.location === deletedPath;
}

export function oracleJavaCompileErrorResult(selectedOracleSourceFile, errorMessage = "") {
    return {
        success: false,
        scope: "oracle-source",
        targetName: selectedOracleSourceFile?.name,
        message: errorMessage || "Java oracle compilation failed.",
        diagnostics: []
    };
}

export function oracleDslGenerationErrorResult(errorMessage = "") {
    return {
        success: false,
        message: errorMessage || "DSL generation failed.",
        generatedJavaPath: "",
        diagnostics: []
    };
}
