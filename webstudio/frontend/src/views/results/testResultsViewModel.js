...
// Implements WS-FUNC-TEST-RESULTS-001 and WS-UX-TEST-RESULTS-001:
// derives Test Results summaries, filters, execution mode labels, and refreshed selections.
...
export function refreshedResultSelectionState(refreshedResults, preferredGroupPath = "") {
    const resultGroups = refreshedResults?.groups || [];
    const selectedResultGroup = resultGroups.length > 0
        ? resultGroups.find((resultGroup) => resultGroup.path === preferredGroupPath) || resultGroups[resultGroups.length - 1]
        : null;
