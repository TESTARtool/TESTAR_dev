...
export const ROLE_STORAGE_KEY = "testar-webstudio-role";
...
export function storedWebStudioRole(storage) {
    ...
    return normalizeWebStudioRole(storage.getItem(ROLE_STORAGE_KEY));
}

export function storeWebStudioRole(storage, role) {
    if (!storage) {
        return;
    }

    storage.setItem(ROLE_STORAGE_KEY, normalizeWebStudioRole(role));
}
