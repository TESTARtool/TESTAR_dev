...
// Verifies WS-FUNC-TOP-NAV-ROLES-001 and WS-UX-TOP-NAV-ROLES-001:
// role normalization, page mapping, and persistence helpers.
import {
...
} from "../../src/app/webStudioRoles.js";
...
test("reads stored role from storage with advanced fallback", () => {
    const storage = {
        getItem(key) {
            ...
            return WEB_STUDIO_ROLES.BASIC;
        }
    };

    assert.equal(storedWebStudioRole(storage), WEB_STUDIO_ROLES.BASIC);
    assert.equal(storedWebStudioRole(null), WEB_STUDIO_ROLES.ADVANCED);
});

test("stores normalized role in storage", () => {
    const writes = [];
    const storage = {
        setItem(key, value) {
    ...
    storeWebStudioRole(storage, "unknown");

    assert.deepEqual(writes, [
        {
            key: ROLE_STORAGE_KEY,
            value: WEB_STUDIO_ROLES.ADVANCED
        }
    ]);
});
