// Verifies WS-FUNC-TEST-RESULTS-001 and WS-UX-TEST-RESULTS-001:
// Test Results summaries, filters, labels, and refreshed selection state.
...
import {
    ...
    refreshedResultSelectionState,
    resultGroupMode,
    sortedResultGroups,
    summarizeResultGroup
} from "../../../src/views/results/testResultsViewModel.js";
...
test("selects preferred refreshed result group and clears selected file", () => {
    ...
    assert.deepEqual(refreshedResultSelectionState(refreshedResults, "older"), {
        resultsData: refreshedResults,
        selectedResultGroup: { name: "older", path: "older" },
        selectedResultFile: null
    });
});

test("falls back to latest refreshed result group", () => {
    const refreshedResults = {
        groups: [
            { name: "older", path: "older" },
    ...
    assert.deepEqual(refreshedResultSelectionState(refreshedResults, "missing"), {
        resultsData: refreshedResults,
        selectedResultGroup: { name: "latest", path: "latest" },
        selectedResultFile: null
    });
});

test("clears refreshed result selection when no groups exist", () => {
    const refreshedResults = {
        groups: []
    };

    assert.deepEqual(refreshedResultSelectionState(refreshedResults), {
        resultsData: refreshedResults,
