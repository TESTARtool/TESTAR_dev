...
} from "../../../src/views/oracles/testOraclesModel.js";
...
test("builds cleared and loaded oracle source state", () => {
    assert.deepEqual(clearedOracleSourceState(), {
    ...
    const sourceFile = {
        location: "web.testar",
        content: "assert button"
    };

    assert.deepEqual(loadedOracleSourceState(sourceFile), {
        selectedOracleSourceFile: sourceFile,
        oracleSourceDraftContent: "assert button",
        savedOracleSourceContent: "assert button",
        oracleDslResult: null,
        javaCompileResult: null
    });
});

test("detects when deleted oracle source clears the editor", () => {
    assert.equal(
        shouldClearOracleSourceAfterDelete({ location: "web.testar" }, "web.testar"),
        true
    );
    assert.equal(
        shouldClearOracleSourceAfterDelete({ location: "web.testar" }, "other.testar"),
        false
    );
});

test("builds oracle source error result objects", () => {
    assert.deepEqual(oracleJavaCompileErrorResult({ name: "WebOracle.java" }, "Compile failed"), {
        success: false,
        scope: "oracle-source",
        targetName: "WebOracle.java",
        message: "Compile failed",
        diagnostics: []
    });
    assert.deepEqual(oracleDslGenerationErrorResult("DSL failed"), {
        success: false,
        message: "DSL failed",
        generatedJavaPath: "",
        diagnostics: []
