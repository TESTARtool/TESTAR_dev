...
import org.testar.windows.exceptions.GDIException;
import org.testar.windows.exceptions.UIAException;
...
public final class Windows {
    ...
    public static long GetPrimaryMonitorHandle() {
        return MonitorFromPoint(0, 0, MONITOR_DEFAULTTOPRIMARY); /* because point {0,0} is always on the primary monitor!! */
    }
    ...
    public static native long IApplicationActivationManager_ActivateApplication(long pAppActMngr, String appUserModelId, String arguments, int options) throws UIAException;
    ...
    static {
        try {
            File workingDirectoryLibrary = new File("windows.dll");
            String dirname = System.getenv("DIRNAME");
            File dirnameLibrary = dirname == null ? null : new File(dirname, "windows.dll");

            if (workingDirectoryLibrary.exists()) {
                loadExternalLib(workingDirectoryLibrary.getPath());
            } else if (dirnameLibrary != null && dirnameLibrary.exists()) {
                loadExternalLib(dirnameLibrary.getPath());
            } else {
    ...
    private static void loadLib(String name) throws IOException {
        ...
        if (in == null && !name.startsWith("/")) {
            in = Windows.class.getResourceAsStream("/" + name);
        }
        if (in == null) {
            throw new IOException("Unable to locate native library resource: " + name);
        }
