...
public class TestWebVisualImageAspectRatioOracle extends WorkspaceOracleTestSupport {

    @Test
    public void detectsDistortedAspectRatio() {
        Oracle oracle = loadWorkspaceOracle();
        State state = stateWithWidgets(imageWidget(400L, 200L, 200L, 120L, "image.png"));
        ...
        assertTrue(verdicts.get(0).info().contains("Detected distorted image aspect ratio."));
        assertTrue(verdicts.get(0).info().contains("Natural size: 400 x 200 px"));
        assertTrue(verdicts.get(0).info().contains("displayed size: 200 x 120 px"));
        assertEquals(1, verdicts.get(0).visualizer().getShapes().size());
        assertFalse(oracle.isVacuousPass());
    }

    @Test
    public void acceptsProportionalDownscale() {
        assertApplicableImageIsOk(imageWidget(400L, 200L, 200L, 100L, "image.png"));
    }

    @Test
    public void acceptsProportionalUpscale() {
        assertApplicableImageIsOk(imageWidget(400L, 200L, 800L, 400L, "image.png"));
    }

    @Test
    public void acceptsSmallRoundingDifferenceWithinTolerance() {
        assertApplicableImageIsOk(imageWidget(100L, 100L, 101L, 99L, "image.png"));
    }

    @Test
    public void detectsDifferenceBeyondTolerance() {
        Oracle oracle = loadWorkspaceOracle();
        State state = stateWithWidgets(imageWidget(100L, 100L, 103L, 97L, "image.png"));
    ...
    private Oracle loadWorkspaceOracle() {
        return loadWorkspaceOracle("WebVisualImageAspectRatioOracle");
    }
