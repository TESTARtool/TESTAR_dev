...
public class Main {
	...
	private static final String RESOURCES_SETTINGS = "./resources/settings";
	public static String getResourcesSettings() {
	...
	private static void settingsSelection(){
		...
		for (File f : new File(RESOURCES_SETTINGS).listFiles()){
		...
		if (sutSettings.isEmpty())
	    	...
	    	try {
	    		File f = new File(RESOURCES_SETTINGS + "/" + sse);
				if (f.createNewFile()){
	...
	public static String[] getSSE(){
		return new File(RESOURCES_SETTINGS).list(new FilenameFilter(){
			@Override
	...
	public static void main(String[] args) throws IOException{
		...
		if (files != null && files.length > 1){
			...
			for (String f : files){
				File ffile  = new File(RESOURCES_SETTINGS + "/" + f);
				System.out.println("Delete file <" + f + "> = " + ffile.delete());
			}
			files = null;
		}
		...
		String testSettings = RESOURCES_SETTINGS + "/" + SSE_ACTIVATED + "/" + SETTINGS_FILE;
