 ...
 * No generic's or sub-classing have been used, because these constructs do not quite fit with
...
public class XPath {
  ...
  public List<Widget> getXpathResult(ProtocolProxy proxy, String xpathExpr) {
    ...
    try {
      for (String concreteID: Utils.getXPathResult(Utils.getStateXML(
          proxy, state, imageFiles, xpathExpr.contains("ocr()")), xpathExpr)) {
  ...
  public boolean getXPathBooleanResult(ProtocolProxy proxy, String xpathExpr) {
    ...
    List<String> imageFiles = updateImageRecognitionAndOCR(proxy, xpathExpr);
    Boolean result = null;
    result = Utils.getXPathBooleanResult(Utils.getStateXML(
        proxy, state, imageFiles, xpathExpr.contains("ocr()")), xpathExpr);
  ...
  public double getXPathNumberResult(ProtocolProxy proxy, String xpathExpr) {
    ...
    List<String> imageFiles = updateImageRecognitionAndOCR(proxy, xpathExpr);
    Double result = null;
    result = Utils.getXPathNumberResult(Utils.getStateXML(
         proxy, state, imageFiles, xpathExpr.contains("ocr()")), xpathExpr);
  ...
  public String getXPathStringResult(ProtocolProxy proxy, String xpathExpr) {
    ...
    List<String> imageFiles = updateImageRecognitionAndOCR(proxy, xpathExpr);
    String result = null;
    result = Utils.getXPathStringResult(Utils.getStateXML(
        proxy, state, imageFiles, xpathExpr.contains("ocr()")), xpathExpr);
    xpathStringMap.putIfAbsent(xpathExpr, result);
    return result;
  }

  private List<String> updateImageRecognitionAndOCR(
      ProtocolProxy proxy, String xpathExpr) {
