...
public class Image {
   ...
   * @param widget to be recognized widget
   * @param imageFile name of the file with the reference image
   * @return confidence value between 0 and 1 that indicates the level of confidence
   *      (1 is highest level)
   */
  ...
  // SikuliX image recognition
  private Double imageRecognition(
      ProtocolProxy proxy, BufferedImage refShot, BufferedImage widgetShot) {
    double confidence = 0;
    if (!invalidSikuliXInstallation) {
      try {
        Pattern pattern =
            new Pattern(refShot).similar(proxy.getSettings()
                .get(ConfigTags.ConfidenceThreshold).floatValue());
        ...
        while (finder.hasNext()) {
          ...
          if (((match.w * match.h)
              / (widgetShot.getWidth() * widgetShot.getHeight()) * percentageFactor)
              >= proxy.getSettings().get(ConfigTags.MinimumPercentageForImageRecognition)) {
        ...
        if (firstTime) {
          // dirty workaround: SikuliX installation might be invalid,
          // in that case the SikuliX setup has to be executed manually
          // image recognition will continue with the standard Testar compareImage
          firstTime = false;
          invalidSikuliXInstallation = true;
          LogSerialiser.log("Invalid SikuliX installation, "
              + "switching to AWTCanvas.compareImage()\n", LogSerialiser.LogLevel.Info);
        } else {
          throw(t);
        }
      }
