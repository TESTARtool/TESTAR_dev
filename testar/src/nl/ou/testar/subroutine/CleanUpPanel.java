...
public class CleanUpPanel extends JPanel {
  ...
  private static final int GAP_X = 19;
  private static final int HOOGTE = 20;
  private static final int BREEDTE = 192;
  private static final int START_Y = 85;
  private static final int START_SETTINGS_X = 50;
  private static final int START_OUTPUT_X = 250;
  private static final int START_TARGET_X = 410;
  ...
  private String rootTargetFolderName;
  private String[] targetDirs;
  private int numTargetDirs;
  private JCheckBox[] targetBox;
  ...
  private void setDirs() {
    ...
    targetDirs = new File(rootTargetFolderName).list(new FilenameFilter() {
      @Override
      public boolean accept(File current, String name) {
        return !name.toLowerCase().endsWith(OUTPUT_TYPE_LOG);
      }
    ...
    if (targetDirs != null) {
    this.numTargetDirs = targetDirs.length;
    } else 
      this.numTargetDirs = 0;
  }
  ...
  private void setEnabledAndVisible() {
    ...
    boolean targetBool = setupTargetDirs();

    if (settingsBool && outputBool && targetBool) {
  ...
  public void populateFrom(final Settings settings) {
    ...
    this.rootTargetFolderName = "./target/resources/main/output/";
    ...
    JLabel targetLabel = new JLabel(rootTargetFolderName);
    targetLabel.setBounds(START_TARGET_X - 2 * GAP_X, START_Y - 2 * DELTA, BREEDTE, HOOGTE);
    add(targetLabel);
  ...
  // Set up target dir layout
  private boolean setupTargetDirs() {
    int emptyAll = 0;
    targetBox = new JCheckBox[numTargetDirs + 2];

    // root folder
    targetBox[0] = new JCheckBox("root");
    targetBox[0].setBounds(START_TARGET_X, START_Y, BREEDTE, HOOGTE);
    if (isEmptyTargetRoot()) {
      targetBox[0].setEnabled(false);
      emptyAll++;
    } else {
      targetBox[0].setEnabled(true);
    }

    // sub folders
    for (int i = 1; i < numTargetDirs + 1; i++) {
      targetBox[i] = new JCheckBox(targetDirs[i - 1]);
      targetBox[i].setBounds(START_TARGET_X, START_Y + i * DELTA, BREEDTE, HOOGTE);
      if (isEmptyTargetDir(i)) {
        targetBox[i].setEnabled(false);
        emptyAll++;
      } else {
        targetBox[i].setEnabled(true);
      }
    }

    // all folders
    targetBox[numTargetDirs + 1] = new JCheckBox("Select all");
    targetBox[numTargetDirs + 1]
        .setBounds(START_TARGET_X - GAP_X, START_Y - DELTA, BREEDTE, HOOGTE);
    boolean empty = (emptyAll == numTargetDirs + 1);
    if (empty) {
      targetBox[numTargetDirs + 1].setEnabled(false);
    } else {
      targetBox[numTargetDirs + 1].setEnabled(true);
      targetBox[numTargetDirs + 1].setSelected(true);
    }

    // action listeners
    for (int i = 0; i <= numTargetDirs + 1; i++) {
      int m = i;
      targetBox[i].addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent evt) {
          actionTargetCheckBox(m);
        }
      });
      add(targetBox[i]);
    }
  ...
  private void actionSettingsCheckBox(int i) {
    ...
    if (i != (numSettingsDirs + 1)) {
      outputBox[numTargetDirs + 1].setSelected(false);
      settingsBox[numSettingsDirs + 1].setSelected(false);
    }
  }

  private void actionOutputCheckBox(int i) {
    boolean allChecked = outputBox[numTargetDirs + 1].isSelected();
    if (i == numTargetDirs + 1 && allChecked) {
      for (int j = 0; j < numTargetDirs + 1; j++) {
        outputBox[j].setSelected(false);
      }
    }
    if (i != (numTargetDirs + 1)) {
      outputBox[numTargetDirs + 1].setSelected(false);
      settingsBox[numSettingsDirs + 1].setSelected(false);
    }
  }
  
  private void actionTargetCheckBox(int i) {
    boolean allChecked = targetBox[numTargetDirs + 1].isSelected();
    if (i == numTargetDirs + 1 && allChecked) {
      for (int j = 0; j < numTargetDirs + 1; j++) {
        targetBox[j].setSelected(false);
      }
    }
    if (i != (numTargetDirs + 1)) {
      outputBox[numTargetDirs + 1].setSelected(false);
  ...
  private void actionButton() {
    ...
    boolean allTargetChecked = targetBox[numTargetDirs + 1].isSelected();
    for (int i = 0; i < numTargetDirs + 1; i++) {
      // all folders is selected or the specified folder is selected
      if (targetBox[i].isSelected() || allTargetChecked) {
        cleanTargetDir(i);
        targetBox[i].setEnabled(false);
        targetBox[i].setSelected(false);
      }
    }
    ...
    int allTargetEmpty = 0;
    for (int i = 0; i < numTargetDirs + 1; i++) {
      if (!targetBox[i].isEnabled()) {
        allTargetEmpty++;
      }
    }
    ...
    if (allTargetEmpty == numTargetDirs + 1) {
      targetBox[numTargetDirs + 1].setEnabled(false);
    }
    ...
    if (allSettingsEmpty == numSettingsDirs + 1
        ...
        && allTargetEmpty == numTargetDirs + 1
        ) {
      btnDelete.setEnabled(false);
      btnEmpty.setEnabled(true);
    }
  ...
  private boolean isEmptyTargetDir(int i) {
    String folderName;
    if (i == 0) {
      folderName = rootTargetFolderName;
    } else {
      folderName = rootTargetFolderName + targetDirs[i - 1];
    }
    File[] files =
        new File(folderName).listFiles(new FilenameFilter() {
          @Override
  ...
  private boolean isEmptyTargetRoot() {
    File[] files =
        new File(rootTargetFolderName).listFiles(new FilenameFilter() {
          @Override
          public boolean accept(File current, String name) {
            return name.toLowerCase().endsWith(OUTPUT_TYPE_CSV);
          }
  ...
  private void cleanTargetDir(int i) {
    String folderName;
    if (i == 0) {
      folderName = rootTargetFolderName;
    } else {
