...
public class ReinforcementLearningUtil {
  ...
  private static RandomActionSelector<AbstractState, AbstractAction> randomActionSelector = new RandomActionSelector<>();
  ...
  /**
     ...
     */
    public static AbstractAction selectAction(final AbstractState state, final Collection<AbstractAction> actionsSelected) {
        if (actionsSelected.isEmpty()) {
            ...
            return randomActionSelector.selectAction(state, actionSetMaxValues);
        }
    }

    public static Tag<Float> getTag(final Settings settings) {

        String tagName = settings.get(ConfigTags.TagName);
        return (Tag<Float>) RLTags.getTag(tagName);
    }
}
