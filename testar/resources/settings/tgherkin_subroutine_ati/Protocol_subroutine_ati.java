 ...
 * @Date October 2018
 */
public class Protocol_subroutine_ati extends SubroutineProtocol implements SubroutineProxy {
    ...
    */
	...
	public Set<Action> finishState(State state) {
		...
		if (exitUrl != null) {
			...
			for (Widget widget : getTopWidgets(state)) {
				...
				if (title.equalsIgnoreCase(ADDRESSTITLE)) {
		...
		*/
		...
		if (action != null) {
		    System.out.println("[Protocol_subroutine_ATI] " + action.toShortString());
		} else {
			System.out.println("[Protocol_subroutine_ATI] lege action");			
		}
		actions.add(action);
		return actions;
	}
	...
	public void finishSubroutine(State state) {
		...
		exitUrl = getSubData().get(getActualIndexSD())[2];
	}
