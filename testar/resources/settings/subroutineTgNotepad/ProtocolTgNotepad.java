 ...
 * for a Notepad desktop application.
 *
 * @author Conny Hageluken
 * @Date January 2019
 */
public class ProtocolTgNotepad
    extends TgherkinFileProtocol {

  /**
   * Default constructor.
   */
  public ProtocolTgNotepad() {
    //default constructor
  }

  /**
   * The state is fulfilling the criterion for running a subroutine.
  ...
  public boolean startState(State state) {
    ...
    if (role != null && "UIAMenuItem".equalsIgnoreCase(role)) {
      for (Integer index: getInputData().keySet()) {
        String selected = getInputData().get(index)[0];
