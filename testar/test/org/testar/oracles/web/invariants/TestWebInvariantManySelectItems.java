...
public class TestWebInvariantManySelectItems {

	private static List<Oracle> extendedOraclesList = new ArrayList<>();

	@BeforeClass
	public static void setup_web_invariant_many_select_items() {
		List<Pair<?, ?>> tags = new ArrayList<Pair<?, ?>>();
		tags.add(Pair.from(ConfigTags.ExtendedOracles, "WebInvariantManySelectItems"));
	...
	public void test_detection_web_invariant_many_select_items() {
		...
		try (MockedStatic<WdDriver> mockedStatic = Mockito.mockStatic(WdDriver.class)) {
			...
			Assert.isTrue(extendedOraclesList.get(0) instanceof WebInvariantManySelectItems);
			...
			Assert.isTrue(verdict.info().equals("Detected Select widgets 'selectid' ,  which have more items than the threshold value of 100"));
		}
	}
